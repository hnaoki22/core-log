...
interface Props {
  token: string;
  initialData: ManagerHomeInitialData;
}

export default function ManagerHomeClient({ token, initialData }: Props) {
  const { isOn } = useFeatures();
  // Hydrate from server data — no client fetch waterfall on first paint.
  ...
  // Background refresh: keep data fresh in long-lived tabs; burnout scores
  // are too expensive to fetch on the server (LLM calls per participant),
  // so they remain client-side gated by feature flag.
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        ...
        if (cancelled || !res.ok) {
          if (!cancelled && !res.ok) setError("上司が見つかりません");
          return;
        }
        ...
        if (isOn("tier-a.burnoutScore")) {
          try {
            ...
            if (cancelled || !burnoutRes.ok) return;
  ...
  // Error fallback — only reachable if the background refresh fails AND
  // the initial server-rendered data was somehow cleared.
  if (error || !data) {
    return (
